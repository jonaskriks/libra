...
impl TryFrom<&ContractEvent> for ToXDMExchangeRateUpdateEvent {
    type Error = Error;

    fn try_from(event: &ContractEvent) -> Result<Self> {
        if event.type_tag != TypeTag::Struct(ToXDMExchangeRateUpdateEvent::struct_tag()) {
            anyhow::bail!("Expected ToXDMExchangeRateUpdateEvent")
        }
        Self::try_from_bytes(&event.event_data)
    }
}
