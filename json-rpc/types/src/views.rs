...
use diem_types::{
    account_config::{
        ...
        ReceivedMintEvent, ReceivedPaymentEvent, SentPaymentEvent, ToXDMExchangeRateUpdateEvent,
    },
...
pub enum EventDataView {
    ...
    #[serde(rename = "to_xdm_exchange_rate_update")]
    ToXDMExchangeRateUpdate {
        currency_code: String,
        new_to_xdm_exchange_rate: f32,
    },
...
impl TryFrom<ContractEvent> for EventDataView {
    ...
    fn try_from(event: ContractEvent) -> Result<Self> {
        ...
        } else if event.type_tag() == &TypeTag::Struct(ToXDMExchangeRateUpdateEvent::struct_tag()) {
            let update_event = ToXDMExchangeRateUpdateEvent::try_from(&event)?;
            EventDataView::ToXDMExchangeRateUpdate {
                currency_code: update_event.currency_code().to_string(),
                new_to_xdm_exchange_rate: update_event.new_to_xdm_exchange_rate(),
            }
...
impl From<&CurrencyInfoResource> for CurrencyInfoView {
    fn from(info: &CurrencyInfoResource) -> CurrencyInfoView {
        CurrencyInfoView {
            ...
            to_xdm_exchange_rate: info.exchange_rate(),
