...
/// Updates the on-chain exchange rate from the given `Currency` to micro-XDM.  The exchange rate
...
/// | `new_exchange_rate_numerator`   | `u64`     | The numerator for the new to micro-XDM exchange rate for `Currency`.                                                               |
/// | `new_exchange_rate_denominator` | `u64`     | The denominator for the new to micro-XDM exchange rate for `Currency`.                                                             |
///
...
fun update_exchange_rate<Currency>(
    ...
    Diem::update_xdm_exchange_rate<Currency>(tc_account, rate);
}
spec fun update_exchange_rate {
    ...
    include Diem::UpdateXDMExchangeRateAbortsIf<Currency>;
