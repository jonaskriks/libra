...
/// `DualAttestation::get_cur_microdiem_limit` XDM and `payer` and `payee` are distinct VASPs.
...
/// | `Errors::LIMIT_EXCEEDED`   | `DiemAccount::EWITHDRAWAL_EXCEEDS_LIMITS`       | `payer` has exceeded its daily withdrawal limits for the backing coins of XDM.                                                      |
/// | `Errors::LIMIT_EXCEEDED`   | `DiemAccount::EDEPOSIT_EXCEEDS_LIMITS`          | `payee` has exceeded its daily deposit limits for XDM.                                                                              |
///
/// # Related Scripts
