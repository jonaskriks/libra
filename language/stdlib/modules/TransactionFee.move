...
module TransactionFee {
    ...
    /// If the `CoinType` is XDM, it unpacks the coin and preburns the
    /// underlying fiat.
    public fun burn_fees<CoinType>(
        ...
        if (XDM::is_xdm<CoinType>()) {
            // TODO: Once the composition of XDM is determined fill this in to
            // unpack and burn the backing coins of the XDM coin.
    ...
    spec fun burn_fees {
        ...
        include DiemTimestamp::AbortsIfNotOperating;
        aborts_if !is_coin_initialized<CoinType>() with Errors::NOT_PUBLISHED;
        include if (XDM::spec_is_xdm<CoinType>()) BurnFeesXDM else BurnFeesNotXDM<CoinType>;
