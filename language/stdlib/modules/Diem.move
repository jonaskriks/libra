...
module Diem {
    ...
    /// account or the `0x1::XDM` module (and `CoreAddresses::DIEM_ROOT_ADDRESS()` in testnet).
    ...
    /// An `ToXDMExchangeRateUpdateEvent` is emitted every time the to-XDM exchange
    ...
    struct ToXDMExchangeRateUpdateEvent {
        ...
        /// `currency_code` currency and XDM. Represented in conversion
        /// between the (on-chain) base-units for the currency and microdiem.
        new_to_xdm_exchange_rate: u64,
    }
    ...
    resource struct CurrencyInfo<CoinType> {
        ...
        preburn_value: u64,
        /// The (rough) exchange rate from `CoinType` to `XDM`. Mutable.
        to_xdm_exchange_rate: FixedPoint32,
        ...
        exchange_rate_update_events: EventHandle<ToXDMExchangeRateUpdateEvent>,
    }
    ...
    /// the treasury compliance account or the `0x1::XDM` module can acquire such a
    /// reference.
    ...
    ): (MintCapability<CoinType>, BurnCapability<CoinType>)
    {
        ...
        move_to(dr_account, CurrencyInfo<CoinType> {
            ...
            exchange_rate_update_events: Event::new_event_handle<ToXDMExchangeRateUpdateEvent>(dr_account)
        });
    ...
    /// Returns the value of the coin in the `FromCoinType` currency in XDM.
    /// This should only be used where a _rough_ approximation of the exchange
    /// rate is needed.
    public fun approx_xdm_for_value<FromCoinType>(from_value: u64): u64
    acquires CurrencyInfo {
        let xdm_exchange_rate = xdm_exchange_rate<FromCoinType>();
        FixedPoint32::multiply_u64(from_value, xdm_exchange_rate)
    }
    spec fun approx_xdm_for_value {
        pragma opaque;
        include ApproxXdmForValueAbortsIf<FromCoinType>;
        ensures result == spec_approx_xdm_for_value<FromCoinType>(from_value);
    }
    spec schema ApproxXdmForValueAbortsIf<CoinType> {
        from_value: num;
        include AbortsIfNoCurrency<CoinType>;
        let xdm_exchange_rate = spec_xdm_exchange_rate<CoinType>();
        include FixedPoint32::MultiplyAbortsIf{val: from_value, multiplier: xdm_exchange_rate};
    }

    /// Returns the value of the coin in the `FromCoinType` currency in XDM.
    /// This should only be used where a rough approximation of the exchange
    /// rate is needed.
    public fun approx_xdm_for_coin<FromCoinType>(coin: &Diem<FromCoinType>): u64
    acquires CurrencyInfo {
        let from_value = value(coin);
        approx_xdm_for_value<FromCoinType>(from_value)
    }
    ...
    /// Updates the `to_xdm_exchange_rate` held in the `CurrencyInfo` for
    /// `FromCoinType` to the new passed-in `xdm_exchange_rate`.
    public fun update_xdm_exchange_rate<FromCoinType>(
        tc_account: &signer,
        xdm_exchange_rate: FixedPoint32
    ) acquires CurrencyInfo {
        Roles::assert_treasury_compliance(tc_account);
        ...
        Event::emit_event(
            ...
            ToXDMExchangeRateUpdateEvent {
                currency_code: *&currency_info.currency_code,
                new_to_xdm_exchange_rate: FixedPoint32::get_raw_value(*&currency_info.to_xdm_exchange_rate),
            }
        );
    }
    spec fun update_xdm_exchange_rate {
        include UpdateXDMExchangeRateAbortsIf<FromCoinType>;
        include UpdateXDMExchangeRateEnsures<FromCoinType>;
    }
    spec schema UpdateXDMExchangeRateAbortsIf<FromCoinType> {
        tc_account: signer;
    ...
    spec schema UpdateXDMExchangeRateEnsures<FromCoinType> {
        xdm_exchange_rate: FixedPoint32;
        ensures spec_currency_info<FromCoinType>().to_xdm_exchange_rate == xdm_exchange_rate;
    }

    /// Returns the (rough) exchange rate between `CoinType` and `XDM`
    public fun xdm_exchange_rate<CoinType>(): FixedPoint32
    acquires CurrencyInfo {
        assert_is_currency<CoinType>();
        *&borrow_global<CurrencyInfo<CoinType>>(CoreAddresses::CURRENCY_INFO_ADDRESS()).to_xdm_exchange_rate
    }
    ...
    spec schema ExchangeRateRemainsSame<CoinType> {
        ...
        ensures old(spec_is_currency<CoinType>())
            ==> spec_currency_info<CoinType>().to_xdm_exchange_rate
                == old(spec_currency_info<CoinType>().to_xdm_exchange_rate);
    }
    spec module {
        /// The permission "UpdateExchangeRate(type)" is granted to TreasuryCompliance [[H5]][PERMISSION].
        apply Roles::AbortsIfNotTreasuryCompliance{account: tc_account} to update_xdm_exchange_rate<FromCoinType>;
        ...
        apply ExchangeRateRemainsSame<CoinType> to *<CoinType>
            except update_xdm_exchange_rate<CoinType>;
    }
    ...
    spec module {
        ...
        /// Specification version of `Self::approx_xdm_for_value`.
        define spec_approx_xdm_for_value<CoinType>(value: num):  num {
            FixedPoint32::spec_multiply_u64(value, spec_xdm_exchange_rate<CoinType>())
        }

        define spec_xdm_exchange_rate<CoinType>(): FixedPoint32 {
            global<CurrencyInfo<CoinType>>(CoreAddresses::CURRENCY_INFO_ADDRESS()).to_xdm_exchange_rate
        }
