...
module DiemAccount {
    ...
    /// e.g., `Diem<XDM>` to an account that exists, but does not have a `Balance<XDM>` resource
    ...
    fun add_currencies_for_account<Token>(
        ...
        if (add_all_currencies) {
            ...
            if (!exists<Balance<XDM>>(new_account_addr)) {
                add_currency<XDM>(new_account);
            };
        };
    }
    ...
    spec schema AddCurrencyForAccountAbortsIf<Token> {
        ...
        include add_all_currencies && !exists<Balance<Coin1>>(addr)
            ==> Diem::AbortsIfNoCurrency<Coin1>;
        include add_all_currencies && !exists<Balance<XDM>>(addr)
            ==> Diem::AbortsIfNoCurrency<XDM>;
    }
    ...
    spec module {
        ...
        apply BalanceNotDecrease<Token> to *<Token>
            except withdraw_from, withdraw_from_balance, staple_xdm, unstaple_xdm,
