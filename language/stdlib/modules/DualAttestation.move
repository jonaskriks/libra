...
module DualAttestation {
    ...
    fun dual_attestation_required<Token>(
        ...
        let approx_xdm_microdiem_value = Diem::approx_xdm_for_value<Token>(deposit_value);
        let above_limit = approx_xdm_microdiem_value >= travel_rule_limit_microdiem;
        if (!above_limit) {
            return false
        };
    ...
    spec define spec_dual_attestation_required<Token>(
        ...
        Diem::spec_approx_xdm_for_value<Token>(deposit_value)
    ...
    public fun initialize(dr_account: &signer) {
        ...
        let initial_limit = (INITIAL_DUAL_ATTESTATION_LIMIT as u128) * (Diem::scaling_factor<XDM>() as u128);
        assert(initial_limit <= MAX_U64, Errors::limit_exceeded(ELIMIT));
        move_to(
            dr_account,
            Limit {
                micro_xdm_limit: (initial_limit as u64)
            }
        )
    }
    spec fun initialize {
        ...
        let initial_limit = INITIAL_DUAL_ATTESTATION_LIMIT * Diem::spec_scaling_factor<XDM>();
        aborts_if initial_limit > MAX_U64 with Errors::LIMIT_EXCEEDED;
        include Diem::AbortsIfNoCurrency<XDM>; // for scaling_factor.
    }
    ...
    public fun get_cur_microdiem_limit(): u64 acquires Limit {
        ...
        borrow_global<Limit>(CoreAddresses::DIEM_ROOT_ADDRESS()).micro_xdm_limit
    }
    ...
    public fun set_microdiem_limit(tc_account: &signer, micro_xdm_limit: u64) acquires Limit {
        Roles::assert_treasury_compliance(tc_account);
        assert(exists<Limit>(CoreAddresses::DIEM_ROOT_ADDRESS()), Errors::not_published(ELIMIT));
        borrow_global_mut<Limit>(CoreAddresses::DIEM_ROOT_ADDRESS()).micro_xdm_limit = micro_xdm_limit;
    }
    spec fun set_microdiem_limit {
        ...
        aborts_if !spec_is_published() with Errors::NOT_PUBLISHED;
        ensures global<Limit>(CoreAddresses::DIEM_ROOT_ADDRESS()).micro_xdm_limit == micro_xdm_limit;
    }
    ...
    spec module {
        ...
        define spec_get_cur_microdiem_limit(): u64 {
            global<Limit>(CoreAddresses::DIEM_ROOT_ADDRESS()).micro_xdm_limit
        }
    }
