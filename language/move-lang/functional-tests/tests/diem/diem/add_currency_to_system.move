...
module COIN {
    ...
    public fun initialize(dr_account: &signer, tc_account: &signer) {
        ...
        Diem::register_SCS_currency<COIN>(
            dr_account,
            tc_account,
            FixedPoint32::create_from_rational(1, 2), // exchange rate to XDM
...
fun main(account: &signer) {
    assert(Diem::approx_xdm_for_value<COIN>(10) == 5, 1);
    assert(Diem::scaling_factor<COIN>() == 1000000, 2);
    assert(Diem::fractional_part<COIN>() == 100, 3);
    Diem::update_xdm_exchange_rate<COIN>(account, FixedPoint32::create_from_rational(1, 3));
    assert(Diem::approx_xdm_for_value<COIN>(10) == 3, 4);
}
}
// check: ToXDMExchangeRateUpdateEvent
// check: "Keep(EXECUTED)"
