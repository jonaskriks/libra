...
fun main(account: &signer, amount: u64) {
    ...
    DiemAccount::pay_from<Coin1>(&with_cap, {{carol}}, amount, x"", x"");
...
fun main() {
    assert(DiemAccount::balance<Coin1>({{carol}}) == 1000, 42);
    assert(DiemAccount::balance<Coin1>({{alice}}) == DiemAccount::balance<Coin1>({{bob}}) + 1000, 43)
}
}
// check: "Keep(EXECUTED)"
