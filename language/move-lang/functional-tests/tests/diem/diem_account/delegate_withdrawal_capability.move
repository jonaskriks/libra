...
module SillyColdWallet {
    ...
    public fun withdraw(account: &signer, wallet_address: address, _amount: u64): Diem::Diem<Coin1> acquires T {
...
fun main(account: &signer) {
    ...
    DiemAccount::pay_from<Coin1>(&with_cap, {{alice}}, 1000, x"", x"");
...
fun main(account: &signer) {
    let with_cap = DiemAccount::extract_withdraw_capability(account);
    DiemAccount::pay_from<Coin1>(&with_cap, {{bob}}, 1000, x"", x"");
    DiemAccount::restore_withdraw_capability(with_cap);
}
}
